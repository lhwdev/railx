	...
	class Calculated(val yOffset: Float, val width: Float) {
		...
		companion object {
			val streamCodec = StreamCodec.composite(
				ByteBufCodecs.FLOAT, Calculated::yOffset,
				ByteBufCodecs.FLOAT, Calculated::width,
				::Calculated,
			)
		}
		
		fun applyTo(be: CantileverBlockEntity) {
			val registries = be.level!!.registryAccess()
			
			val nbt = be.saveCustomOnly(registries)
			nbt.putFloat(CantileverBlockEntity.NBT_Y_OFFSET, yOffset)
			nbt.putFloat(CantileverBlockEntity.NBT_WIDTH, width)
			
			be.loadCustomOnly(nbt, registries)
		}
	}
}
