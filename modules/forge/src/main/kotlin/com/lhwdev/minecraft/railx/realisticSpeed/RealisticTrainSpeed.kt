...
class RealisticTrainSpeed(private val train: Train) {
	...
	fun handleApproachTargetSpeed(accelerationMod: Float): Boolean {
	...
	private fun handleRollingResistance() {
		...
		if(currentSpeed == 0.0) {
			...
			resistance *= config.rollingResistanceMultiplier.asDouble
	...
	private fun handleBrake() {
		...
		if(config.automaticBrakeAtStation.isTrue && train.currentStation != null && targetThrottle == 0.0) {
			brake = max(brake, 1.0)
		}
